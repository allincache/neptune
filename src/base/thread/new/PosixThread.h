...
typedef pid_t PosixThreadId;

// Used for thread checking and debugging.
// Meant to be as fast as possible.
// These are produced by PosixThread::CurrentRef(), and used to later
// check if we are on the same thread or not by using ==. These are safe
// to copy between threads, but can't be copied to another process as they
// have no meaning there. Also, the internal identifier can be re-used
// after a thread dies, so a PosixThreadRef cannot be reliably used
// to distinguish a new thread from an old, dead thread.
class PosixThreadRef {
 public:
#if defined(OS_WIN)
...
#endif
  constexpr PosixThreadRef() : id_(0) {}

  explicit constexpr PosixThreadRef(RefType id) : id_(id) {}

  bool operator==(PosixThreadRef other) const {
    return id_ == other.id_;
  }

  bool operator!=(PosixThreadRef other) const { return id_ != other.id_; }
...
class PosixThreadHandle {
 public:
  ...
  constexpr PosixThreadHandle() : handle_(0) {}

  explicit constexpr PosixThreadHandle(Handle handle) : handle_(handle) {}

  bool is_equal(const PosixThreadHandle& other) const {
...
const PosixThreadId kInvalidThreadId(0);
...
class PosixThread {
 public:
  ...
  static PosixThreadId CurrentId();

  // Gets the current thread reference, which can be used to check if
  // we're on the right thread quickly.
  static PosixThreadRef CurrentRef();
  ...
  static PosixThreadHandle CurrentHandle();
  ...
  // |thread_handle|.
  static void Join(PosixThreadHandle thread_handle);

  // Detaches and releases the thread handle. The thread is no longer joinable
  // and |thread_handle| is invalidated after this call.
  static void Detach(PosixThreadHandle thread_handle);
  ...
  static void SetThreadPriority(PosixThreadId thread_id,
                                ThreadPriority priority);
#endif
 ...
 private:
  ...
  //DISALLOW_IMPLICIT_CONSTRUCTORS(PosixThread);
};
...
// synchronized with calls to PosixThread::SetCurrentThreadPriority().
