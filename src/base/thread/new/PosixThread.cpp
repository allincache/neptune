...
struct ThreadParams {
  ...
  PosixThread::Delegate* delegate;
  bool joinable;
  ThreadPriority priority;
};

void* ThreadFunc(void* params) {
  PosixThread::Delegate* delegate = nullptr;

  {
    ...
    // threads.
  //  PosixThread::SetCurrentThreadPriority(thread_params->priority);
#endif
  }

  // ThreadIdNameManager::GetInstance()->RegisterThread(
  //     PosixThread::CurrentHandle().platform_handle(),
  //     PosixThread::CurrentId());
...
bool CreateThread(size_t stack_size,
                  bool joinable,
                  PosixThread::Delegate* delegate,
                  PosixThreadHandle* thread_handle,
                  ThreadPriority priority) {
  ...
  *thread_handle = PosixThreadHandle(handle);
...
// expensive and PosixThread::CurrentId is used liberally. Clear
...
// static
PosixThreadId PosixThread::CurrentId() {
...
// static
PosixThreadRef PosixThread::CurrentRef() {
  return PosixThreadRef(pthread_self());
}

// static
PosixThreadHandle PosixThread::CurrentHandle() {
  return PosixThreadHandle(pthread_self());
}

// static
void PosixThread::YieldCurrentThread() {
  sched_yield();
}

// static
// void PosixThread::Sleep(TimeDelta duration) {
...
// static
const char* PosixThread::GetName() {
  //return ThreadIdNameManager::GetInstance()->GetName(CurrentId());
}

// static
bool PosixThread::CreateWithPriority(size_t stack_size, Delegate* delegate,
                                        PosixThreadHandle* thread_handle,
                                        ThreadPriority priority) {
...
// static
bool PosixThread::CreateNonJoinable(size_t stack_size, Delegate* delegate) {
  return CreateNonJoinableWithPriority(stack_size, delegate,
                                       ThreadPriority::NORMAL);
}

// static
bool PosixThread::CreateNonJoinableWithPriority(size_t stack_size,
                                                   Delegate* delegate,
                                                   ThreadPriority priority) {
  PosixThreadHandle unused;
...
// static
void PosixThread::Join(PosixThreadHandle thread_handle) {
...
// static
void PosixThread::Detach(PosixThreadHandle thread_handle) {
...
// static
bool PosixThread::CanIncreaseThreadPriority(ThreadPriority priority) {
#if defined(OS_NACL)
  return false;
#else
...
// static
void PosixThread::SetCurrentThreadPriorityImpl(ThreadPriority priority) {
#if defined(OS_NACL)
  NOTIMPLEMENTED();
#else
  ...
  //   //           << PosixThread::CurrentId() << ") to " << nice_setting;
  // }
#endif  // defined(OS_NACL)
}

// static
ThreadPriority PosixThread::GetCurrentThreadPriority() {
#if defined(OS_NACL)
  NOTIMPLEMENTED();
...
// static
// size_t PosixThread::GetDefaultThreadStackSize() {
//   pthread_attr_t attributes;
//   pthread_attr_init(&attributes);
