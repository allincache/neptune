...
class ThreadIdNameManager {
 public:
  ...
  void RegisterThread(PosixThreadHandle::Handle handle, PosixThreadId id);
...
//  void InstallSetNameCallback(SetNameCallback callback);
  ...
  const char* GetName(PosixThreadId id);

  // Unlike |GetName|, this method using TLS and avoids touching |lock_|.
  const char* GetNameForCurrentThread();

  // Remove the name for the given id.
  void RemoveName(PosixThreadHandle::Handle handle, PosixThreadId id);

 private:
  friend struct DefaultSingletonTraits<ThreadIdNameManager>;

  typedef std::map<PosixThreadId, PosixThreadHandle::Handle>
      ThreadIdToHandleMap;
  typedef std::map<PosixThreadHandle::Handle, std::string*>
  ...
  // Treat the main process specially as there is no PosixThreadHandle.
  std::string* main_process_name_;
  PosixThreadId main_process_id_;
