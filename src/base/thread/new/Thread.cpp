...
// #endif
 ...
 //   AutoLock lock(thread_lock_);
    bool success =
        options.joinable
            ? PosixThread::CreateWithPriority(options.stack_size, this,
                                                 &thread_, options.priority)
            : PosixThread::CreateNonJoinableWithPriority(
...
//  AutoLock lock(thread_lock_);
  ...
  //
  PosixThread::Join(thread_);
  thread_ = base::PosixThreadHandle();
...
PosixThreadId Thread::GetThreadId() const {
...
//   DCHECK_EQ(id_, PosixThread::CurrentId());
...
void Thread::ThreadMain() {
 ...
 // DCHECK(!id_event_.IsSignaled());
  ...
  id_ = PosixThread::CurrentId();
  //DCHECK_NE(kInvalidThreadId, id_);
 // id_event_.Signal();

  // Complete the initialization of our Thread object.
//  PosixThread::SetName(name_.c_str());
