#ifndef NEP_BASE_STRING_W_BUFFER_H
#define NEP_BASE_STRING_W_BUFFER_H
...
class WarningBufferFactory
{
 public:
  ...
  WarningBuffer* get_buffer() const
  {
    ...
    if (INVALID_THREAD_KEY != key_)
    {
      ...
      if (NULL == ptr)
      {
        ...
        if (NULL != ptr)
        {
          ...
          if (0 != ret)
          {
            // LOG(ERROR, "pthread_setspecific failed:%d", ret);
            free(ptr);
            ptr = NULL;
          }
          else
        ...
        {
          // malloc failed;
          // LOG(ERROR, "malloc thread specific memeory failed.");
        }
      }
      else
      {
    ...
    {
      // LOG(ERROR, "thread key must be initialized "
 ...
 private:
  int create_thread_key()
  {
    ...
    if (0 != ret)
    {
    //  LOG(ERROR, "cannot create thread key:%d", ret);
    }
    return (0 == ret) ? 0 : 1;
  }
  ...
  int delete_thread_key()
  {
    ...
    if (0 != ret)
    {
    //  LOG(ERROR, "delete thread key key_ failed.");
    }
    return (0 == ret) ? 0 : 1;
  }
...
#endif //NEP_BASE_STRING_W_BUFFER_H
