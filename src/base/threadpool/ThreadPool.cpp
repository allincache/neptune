...
#include "base/common/Functional.h"
...
ThreadPool::~ThreadPool()
{
  assert(_destroyed);
  _monitor.lock();
  LOG(DEBUG,"_workItem.size: %d,_listSize: %d,_procSize: %d", _workItems.size(),_listSize,_procSize);
...
int ThreadPool::execute(ThreadPoolWorkItem* workItem)
{
  if ( _destroyed)
  {
    //LOG(ERROR,"%s","ThreadPoolDestroyedException");
    return -1;
  }
  if (_listSize > _listSizeMax )
  {
    //LOG(DEBUG,"%s","ThreadPoolQueueFullException");
    return -1;
  }
