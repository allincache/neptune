...
static const char SMALL_DFS_FILE_KEY_CHAR = 'T';
static const char LARGE_DFS_FILE_KEY_CHAR = 'L';

static const char* KEY_MASK = "NEPTUNE";
...
void FSName::set_name(const char* file_name, const char* suffix, const int32_t cluster_id)
{
  ...
  {
    if (check_file_type(file_name) == INVALID_DFS_FILE_TYPE)
    {
      is_valid_ = false;
    }
    else
...
const char* FSName::get_name(const bool large_flag)
{
  if (file_name_[0] == '\0')
  {
    ...
    if (large_flag)
    {
      file_name_[0] = LARGE_DFS_FILE_KEY_CHAR;
    }
    else
    {
      file_name_[0] = SMALL_DFS_FILE_KEY_CHAR;
    }
...
BaseFileType FSName::check_file_type(const char* dfs_name)
{
  BaseFileType file_type = INVALID_DFS_FILE_TYPE;
  if (NULL != dfs_name &&
      static_cast<int32_t>(strlen(dfs_name)) >= FILE_NAME_LEN)
  {
    if (LARGE_DFS_FILE_KEY_CHAR == dfs_name[0])
    {
      file_type = LARGE_DFS_FILE_TYPE;
    }
    else if (SMALL_DFS_FILE_KEY_CHAR == dfs_name[0])
    {
      file_type = SMALL_DFS_FILE_TYPE;
    }
  }
  return file_type;
}
