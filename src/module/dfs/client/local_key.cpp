...
const char* LOCAL_KEY_PATH = "/tmp/DFSlocalkeyDIR/";
...
int LocalKey::initialize(const char* local_key, const uint64_t addr)
{
  ...
  {
    //LOG(ERROR, "create localkey path %s with mode %d fail, error: %s",
    //          LOCAL_KEY_PATH, LOCAL_KEY_PATH_MODE, strerror(errno));
    ret = ERROR;
  }
  else if ((ret = init_local_key_name(local_key, addr, name)) != SUCCESS)
  {
    //LOG(ERROR, "init local key name fail, ret: %d", ret);
  }
  else
  {
    clear();
    ...
    {
      //LOG(DEBUG, "create new localkey file: %s", name);
      ret = init_file_op(name, O_RDWR|O_CREAT);
    }
    else if ((ret = init_file_op(name, O_RDWR)) == SUCCESS)
    {
      ret = load();
      // load fail. localkey is invalid, just delete
      if (ret != SUCCESS)
      {
        //LOG(WARN, "load local key fail, file is invalid, ignore: %s", name);
        clear();
        ret = SUCCESS;
      }
    }
...
int LocalKey::load()
{
  ...
  if (NULL == file_op_)
  {
    //LOG(ERROR, "local key file path not initialize");
    ret = ERROR;
  }
  else if ((ret = file_op_->pread_file(reinterpret_cast<char*>(&seg_head_), sizeof(SegmentHead), 0)) != SUCCESS)
  {
    //LOG(ERROR, "load segment head fail, %s, ret: %d", file_op_->get_file_name(), ret);
  }
  else
  {
    //LOG(INFO, "load segment count %d, size: %"PRI64_PREFIX"d", seg_head_.count_, seg_head_.size_);
    ...
    {
      //LOG(ERROR, "load segment info fail, %s, ret: %d", file_op_->get_file_name(), ret);
    }
    else
    {
...
{
  int ret = seg_info_.insert(seg_info).second ? SUCCESS : ERROR;
  if (SUCCESS == ret)
  {
    seg_head_.count_++;
    seg_head_.size_ += seg_info.size_;
    //LOG(DEBUG, "add segment successful. blockid: %u, fileid: %"PRI64_PREFIX"u, offset: %"PRI64_PREFIX"d, size: %d, crc: %u",
    //          seg_info.block_id_, seg_info.file_id_, seg_info.offset_, seg_info.size_, seg_info.crc_);
  }
  else
  {
    // add to gc ?
    //LOG(ERROR, "add segment fail. blockid: %u, fileid: %"PRI64_PREFIX"u, offset: %"PRI64_PREFIX"d, size: %d, crc: %u",
    //          seg_info.block_id_, seg_info.file_id_, seg_info.offset_, seg_info.size_, seg_info.crc_);
  }
  return ret;
}
...
int LocalKey::validate(const int64_t total_size)
{
  ...
  {
    //LOG(ERROR, "segment containing size less than required size: %"PRI64_PREFIX"d < %"PRI64_PREFIX"d",
    //          seg_head_.size_, total_size);
    ret = ERROR;
  }
  else if (static_cast<size_t>(seg_head_.count_) != seg_info_.size())
  {
    //LOG(ERROR, "segment count conflict with head meta info count: %d <> %zd",
  ...
  else if (seg_info_.size() > 0)   // not empty
  {
    ...
    if (it->offset_ != 0)
    {
      //LOG(ERROR, "segment info offset not start with 0: %"PRI64_PREFIX"d", it->offset_);
      ret = ERROR;
    }
    else
    {
      ...
      for (; nit != seg_info_.end() && size < check_size; ++it, ++nit)
      {
        ...
        {
          //LOG(ERROR, "segment info conflict: (offset + size != next_offset) %"PRI64_PREFIX"d + %d != %"PRI64_PREFIX"d", it->offset_, it->size_, nit->offset_);
          ret = ERROR;
          break;
        }
        size += nit->size_;
      ...
      if (SUCCESS == ret)
      {
        ...
        if (size != check_size)
        {
          //LOG(ERROR, "segment info conflict: segment size conflict required size: %"PRI64_PREFIX"d, < %"PRI64_PREFIX"d", size, check_size);
          ret = ERROR;
        }
...
int LocalKey::save()
{
  ...
  if (NULL == file_op_)
  {
    //LOG(ERROR, "local save file path not initialize");
    ret = ERROR;
  }
  else
  {
    ...
    if ((ret = file_op_->pwrite_file(buf, size, 0)) != SUCCESS)
    {
      //LOG(ERROR, "save segment info fail, count: %zd, raw size: %d, file size: %"PRI64_PREFIX"d, ret: %d",
      //          seg_info_.size(), size, seg_head_.size_, ret);
    }
    else
    {
      //LOG(INFO, "save segment info successful, count: %zd, raw size: %d, file size: %"PRI64_PREFIX"d, ret: %d",
      //          seg_info_.size(), size, seg_head_.size_, ret);
      file_op_->flush_file();

      if ((ret = gc_file_.save()) != SUCCESS)
      {
        //LOG(ERROR, "save gc file fail, ret: %d", ret);
      }
    }
    ...
    else
    {
      ...
      else
      {
        ...
        while (remain_nw_size > 0 && it != seg_info_.end())
        {
          ...
          {
            //LOG(INFO, "segment size conflict: %d <> %"PRI64_PREFIX"d", it->size_, remain_nw_size);
            need_write_size += remain_nw_size;
            remain_nw_size = 0;
          }
          else if ((tmp_crc = Func::crc(0, cur_buf + need_write_size, it->size_)) != it->crc_)
          {
            //LOG(INFO, "segment crc conflict: %d <> %d", it->crc_, tmp_crc);
            need_write_size += it->size_;
            remain_nw_size -= it->size_;
          }
          else if (DfsClient::Instance()->stat_file(&file_stat, FSName(it->block_id_, it->file_id_).get_name(),
                                                    NULL, NORMAL_STAT, CNetUtil::addrToString(server_id_).c_str())
                   != SUCCESS) // server id util
          {
            //LOG(INFO, "segment info is not valid. blockid: %u, fileid: %"PRI64_PREFIX"u",
          ...
          {
            //LOG(INFO, "segment data written: offset: %"PRI64_PREFIX"d, blockid: %u, fileid: %"PRI64_PREFIX"u, size: %d, crc: %u", it->offset_, it->block_id_, it->file_id_, it->size_, it->crc_);
                                       ...
                                       const int64_t size, SEG_DATA_LIST& seg_list)
{
  ...
  {
    //LOG(INFO, "read file offset not less than size: %"PRI64_PREFIX"d >= %"PRI64_PREFIX"d", offset, seg_head_.size_);
    return 0;
  }
  ...
  if (seg_info_.end() == it || it->offset_ != offset)
  {
    ...
    {
      //LOG(ERROR, "can not find meta info for offset: %"PRI64_PREFIX"d", offset);
                           ...
                           const int64_t size, SEG_DATA_LIST& seg_list)
{
  if (size > 0)
  {
    ...
    while (check_size < size)
    {
      ...
      //LOG(DEBUG, "get segment, seg info size: %d, offset: %"PRI64_PREFIX"d",
...
int LocalKey::load_head(const char* buf, const int32_t buf_len)
{
  ...
  {
    //LOG(ERROR, "buffer length less than base segment head length: %d < %zd", buf_len, sizeof(SegmentHead));
    ret = ERROR;
  }
  else
  {
    memcpy(&seg_head_, buf, sizeof(SegmentHead));
    //LOG(DEBUG, "load segment head, count %d, size: %"PRI64_PREFIX"d", seg_head_.count_, seg_head_.size_);
  }
  return ret;
}
...
int LocalKey::load_segment(const char* buf, const int32_t buf_len)
{
  ...
  {
    //LOG(ERROR, "buffer length less than required segmentInfo. segment count: %d, %d < %zd",
  ...
  else
  {
    ...
    {
      //LOG(DEBUG, "load segment info, offset: %"PRI64_PREFIX"d, blockid: %u, fileid: %"PRI64_PREFIX"u, size: %d, crc: %u",
      ...
      {
        //LOG(ERROR, "load segment info fail, count: %d, failno: %d", count, i + 1);
        ret = ERROR;
        break;
      }
    ...
    if (SUCCESS == ret && size != seg_head_.size_)
    {
      //LOG(ERROR, "segment size conflict with head meta info size: %"PRI64_PREFIX"d <> %"PRI64_PREFIX"d",
...
int LocalKey::init_local_key_name(const char* key, const uint64_t addr, char* local_key_name)
{
  ...
  {
    //LOG(ERROR, "null key or addr occur. key: %p, addr: %"PRI64_PREFIX"u, local key name buffer: %p",
  ...
  else
  {
    ...
    {
      //LOG(ERROR, "initialize local key %s fail, key is not a valid file path. error: %s", key, strerror(errno));
      ret = ERROR;
    }
    else
    {
