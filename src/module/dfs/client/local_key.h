#ifndef N_DFS_CLIENT_LOCALKEY_H_
#define N_DFS_CLIENT_LOCALKEY_H_
...
enum DfsFileEofFlag
{
  DFS_FILE_EOF_FLAG_NO = 0x00,
  DFS_FILE_EOF_FLAG_YES
};
...
struct SegmentData
{
  ...
  union                     // special value in mutex condition
  {
    ...
    DfsUnlinkType unlink_action_;  // unlink as unlink type
    ReadDataOptionFlag read_flag_; // read as read flag
    DfsStatType stat_mode_;        // stat as stat mode
  };
  VUINT64 ds_;
  ...
  SegmentData() : cache_hit_(CACHE_HIT_NONE), delete_flag_(true), buf_(NULL),
                  ...
                  status_(SEG_STATUS_NOT_INIT), eof_(DFS_FILE_EOF_FLAG_NO)
  {
...
SEG->seg_info_.block_id_, SEG->seg_info_.file_id_, SEG->seg_info_.offset_, \
  ...
  CNetUtil::addrToString(SEG->get_last_read_pri_ds()).c_str(), \
  CNetUtil::addrToString(SEG->get_write_pri_ds()).c_str()
...
class LocalKey : public SegmentContainer< std::set<SegmentInfo> >
{
 public:
  LocalKey();
  virtual ~LocalKey();
...
#endif  // N_DFS_CLIENT_LOCALKEY_H_
