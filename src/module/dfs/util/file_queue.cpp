...
int FileQueue::load_queue_head()
{
  ...
  {
    ////LOG(ERROR, "create directoy : %s failed", queue_path_.c_str());
  ...
  {
    ////LOG(ERROR, "open file: %s failed", path.c_str());
  ...
  ////LOG(DEBUG, "readno: %d, writeno: %d, queueSize: %d", queue_information_header_.read_seqno_,
...
{
  if (data == NULL || len <= 0 || len > DFS_MALLOC_MAX_SIZE)
  {
    ////LOG(WARN, "invalid data: %p, length: %"PRI64_PREFIX"d", data, len);
  ...
  *((int32_t*) buffer) = size;
  ...
  if (ret != size)
  {
    ////LOG(WARN, "write filed, path: %s, fd: %d, lenght: %"PRI64_PREFIX"d, ret: %d, size: %d", queue_path_.c_str(), write_fd_,
    //    len, ret, size);
    ret = size - ret;
    if (ret > 0 && ret <= size)
    {
  ...
  //LOG(DEBUG, "queue_size_: %d,size: %d,iret: %d, write_filesize_: %d, max_file_size: %u, write_seqno_: %d",
...
QueueItem *FileQueue::pop(int index)
{
  ...
  if (read_fd_ == -1)
  {
    ////LOG(WARN, "file %s:%d is close...", queue_path_.c_str(), queue_information_header_.read_seqno_);
    do
    {
      ++count;
      iret = open_read_file();
      if (iret != SUCCESS)
      {
        ////LOG(WARN, "reopen file %s: %d failed", queue_path_.c_str(), queue_information_header_.read_seqno_);
        ::close( read_fd_);
        read_fd_ = -1;
        continue;
      }
  ...
  while (true)
  {
    ...
    if (retsize < 0)
    {
      ////LOG(ERROR, "read head size errors %d : %s from : %d", errno, strerror(errno), read_fd_);
      break;
    }
    if (retsize != sizeof(int32_t) && retsize != 0)
    {
      ////LOG(ERROR, "read head size: %d errors %d : %s from : %d", retsize, errno, strerror(errno), read_fd_);
...
int FileQueue::clear()
{
  ...
  queue_information_header_.write_filesize_ = 0;
  queue_information_header_.queue_size_ = 0;
  if ((open_write_file() != SUCCESS) || (open_read_file() != SUCCESS) || (write_header() != SUCCESS))
  {
  //  //LOG(ERROR, "file queue call clear error");
  }
  return ERROR;
}
...
int FileQueue::open_write_file()
{
  ...
  if (write_fd_ == -1)
  {
    //LOG(ERROR, "open file: %s failed", tmp);
    return ERROR;
  }
  return SUCCESS;
}
...
int FileQueue::open_read_file()
{
  ...
  if (iret < 0)
  {
  //  //LOG(ERROR, "call lseek filed: %s", strerror(errno));
    return ERROR;
  }
  return SUCCESS;
}
...
{
  int iret = information_fd_ < 0 ? ERROR : SUCCESS;
  if (iret == SUCCESS)
  {
    int32_t count = 0;
    do
    {
      ...
      ////LOG(ERROR, "write queue infromation header failed, path: %s, errors: %s", queue_path_.c_str(), strerror(
      //    errno));
      ::close( information_fd_);
      information_fd_ = -1;
      char tmp[MAX_PATH_LENGTH+1];
...
int FileQueue::recover_record()
{
  ...
  ////LOG(INFO, "%s: recoverRecord: %d", queue_path_.c_str(), count);
  memset(&queue_information_header_.pos_[0], 0, FILE_QUEUE_MAX_THREAD_SIZE * sizeof(unsettle));
  return write_header();
}
