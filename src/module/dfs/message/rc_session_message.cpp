...
int ReqRcLoginMessage::set_app_key(const char* app_key)
{
  ...
  if (NULL == app_key)
  {
    ...
    LOG(ERROR, "set_app_key fail, app_key is null, ret: %d", ret);
  }
  else
  {
    ...
    {
      ret = EXIT_INVALID_ARGU;
      LOG(ERROR, "set_app_key fail. app_key: %s, len: %d, ret: %d", app_key, length_, ret);
    }
    else
    {
...
int ReqRcLoginMessage::serialize(Stream& output) const
{
  int ret = SUCCESS;
...
int ReqRcLoginMessage::deserialize(Stream& input)
{
  ...
  if (SUCCESS == ret)
  {
    ...
    {
      ret = ERROR;
      LOG(ERROR, "error msg length: %d invalid. define length: %d", length_, MAX_ERROR_MSG_LENGTH);
    }
    else
    {
...
{
  LOG(DEBUG, "app_key: %s, app_ip: %" PRI64_PREFIX "d", app_key_, app_ip_);
}
...
int RspRcLoginMessage::set_session_id(const char* session_id)
{
  ...
  {
    ret = EXIT_INVALID_ARGU;
    LOG(ERROR, "set_session_id fail, session_id is null, ret: %d", ret);
  }
  else
  {
    ...
    {
      ret = EXIT_INVALID_ARGU;
      LOG(ERROR, "set_session_id fail. session_id: %s, len: %d, ret: %d", session_id, length_, ret);
    }
    else
    {
...
int RspRcLoginMessage::serialize(Stream& output) const
{
  int ret = SUCCESS;
  if (length_ > 0)
  {
...
{
  LOG(DEBUG, "session_id: %s", session_id_);
  base_info_.dump();
}
...
int ReqRcKeepAliveMessage::serialize(Stream& output) const
{
  int ret = SUCCESS;
  int64_t pos = 0;
...
int RspRcKeepAliveMessage::serialize(Stream& output) const
{
  int ret = SUCCESS;
  ret = output.set_int8(update_flag_);
  if (SUCCESS == ret && update_flag_)
  {
    int64_t pos = 0;
    ret = base_info_.serialize(output.get_free(), output.get_free_length(), pos);
    if (SUCCESS == ret)
...
void ReqRcReloadMessage::set_reload_type(const ReloadType type)
{
  reload_type_ = type;
}

int ReqRcReloadMessage::serialize(Stream& output) const
{
...
ReloadType ReqRcReloadMessage::get_reload_type() const
{
  return reload_type_;
}
...
{
  LOG(DEBUG, "reload_type : %d", reload_type_);
  return;
}
