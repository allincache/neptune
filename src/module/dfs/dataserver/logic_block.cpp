...
int LogicBlock::close_write_file(const uint64_t inner_file_id, DataFile* datafile, const uint32_t crc)
{
  ...
  if (SUCCESS == ret)
  {
    //LOG(INFO, "file exist, update! blockid: %u, fileid: %" PRI64_PREFIX "u", logic_block_id_, inner_file_id);
  }
  ...
  FileInfo dfs_file_info, old_file_info;
  dfs_file_info.id_ = inner_file_id;
  dfs_file_info.size_ = file_size + sizeof(FileInfo);
  dfs_file_info.flag_ = 0;
  dfs_file_info.modify_time_ = time(NULL);
  dfs_file_info.create_time_ = time(NULL);
  dfs_file_info.crc_ = crc;
  ...
  {
    //LOG(INFO, "write file. fileid equal. blockid: %u, fileid: %" PRI64_PREFIX "u", logic_block_id_,
    //    inner_file_id);

    ret = data_handle_->read_segment_info(&old_file_info, file_meta.get_offset());
    if (SUCCESS != ret)
    {
      //LOG(ERROR, "read FileInfo fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d, offset: %d", logic_block_id_,
      //    inner_file_id, ret, file_meta.get_offset());
    }
    else // read successful
    {
      // use old time
      dfs_file_info.create_time_ = old_file_info.create_time_;
      ...
      if (require_size > old_file_info.usize_)
      {
        ...
        dfs_file_info.offset_ = block_offset;
        dfs_file_info.usize_ = require_size;
      ...
      else
      {
        ...
        commit_offset = false;
        dfs_file_info.offset_ = file_meta.get_offset();
        dfs_file_info.usize_ = old_file_info.usize_;
      }
      // modify meta size
  ...
  if (file_meta.get_file_id() != inner_file_id || ret)
  {
    ...
    old_size = 0;
    dfs_file_info.usize_ = file_size + sizeof(FileInfo);
    block_offset = index_handle_->get_block_data_offset();
    dfs_file_info.offset_ = block_offset;
  ...
  do
  {
    ...
    while ((tmp_data_buffer = datafile->get_data(NULL, &read_len, read_offset)) != NULL)
    {
      ...
      {
        //LOG(ERROR, "getdata fail, blockid: %u, fileid: %" PRI64_PREFIX "u, size: %d, offset: %d, rlen: %d",
      ...
      if (0 == read_offset)
      {
        ...
        memcpy(tmp_write_buffer, &dfs_file_info, sizeof(FileInfo));
  ...
  //LOG(DEBUG, "close write file, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d", logic_block_id_,
  //    inner_file_id, ret);
  if (SUCCESS != ret) // error occur
  {
    if (need_update_meta)
    {
      // rollback
...
int LogicBlock::read_file(const uint64_t inner_file_id, char* buf, int32_t& nbytes, const int32_t offset, const int8_t flag)
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "blockid: %u read file meta fail, fileid: %" PRI64_PREFIX "u, ret: %d", logic_block_id_,
    //    inner_file_id, ret);
    return ret;
  }
  ...
  if (nbytes < 0)
  {
    //LOG(ERROR, "blockid: %u, fileid: %" PRI64_PREFIX "u, read offset: %d, file size: %d, ret: %d",
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "blockid: %u read data error, fileid: %" PRI64_PREFIX "u, size: %d, offset: %d, ret: %d",
    //    logic_block_id_, inner_file_id, nbytes, offset, ret);
    return ret;
  }

  //LOG(DEBUG, "blockid: %u read data, fileid: %" PRI64_PREFIX "u, read size: %d, offset: %d, ret: %d",
  //    logic_block_id_, inner_file_id, nbytes, offset, ret);
...
int LogicBlock::rename_file(const uint64_t old_inner_file_id, const uint64_t new_inner_file_id)
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "read FileInfo fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d", logic_block_id_,
...
int LogicBlock::unlink_file(const uint64_t inner_file_id, const int32_t action, int64_t& file_size)
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "read segment meta fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d", logic_block_id_,
    //    inner_file_id, ret);
    return ret;
  }
  ...
  case DELETE:
    ...
    {
      //LOG(ERROR, "file already deleted, blockid: %u, fileid: %" PRI64_PREFIX "u", logic_block_id_,
      //    inner_file_id);
  ...
  case UNDELETE:
    ...
    {
      //LOG(ERROR, "file is not deleted, blockid: %u, fileid: %" PRI64_PREFIX "u", logic_block_id_,
      //    inner_file_id);
      return EXIT_FILE_STATUS_ERROR;
    }
  ...
  case CONCEAL:
    ...
    {
      //LOG(ERROR, "file is already deleted or concealed, blockid: %u, fileid: %" PRI64_PREFIX "u",
  ...
  case REVEAL:
    ...
    {
      //LOG(ERROR, "file is not deleted or concealed, blockid: %u, fileid: %" PRI64_PREFIX "u",
      //    logic_block_id_, inner_file_id);
      return EXIT_FILE_STATUS_ERROR;
    }
    finfo.flag_ &= (~FI_CONCEAL);
    break;
  default:
    //LOG(ERROR, "action is illegal. action: %d, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d", action,
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "write segment info fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d", logic_block_id_,
    //    inner_file_id, ret);
    return ret;
  }
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "update block info fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d", logic_block_id_,
    //    inner_file_id, ret);
    return ret;
  }
...
int LogicBlock::read_raw_data(char* buf, int32_t& nbytes, const int32_t offset)
{
  ...
  if (nbytes < 0)
  {
    //LOG(ERROR, "blockid: %u, batch read data offset: %d, block current offset: %d",
...
int LogicBlock::batch_write_meta(const BlockInfo* blk_info, const RawMetaVec* meta_list)
{
  ...
  if (SUCCESS != ret && EXIT_POINTER_NULL != ret)
  {
    return ret;
  }
  //LOG(DEBUG, "batch write meta list, blockid: %u, meta size: %zd", logic_block_id_, meta_list->size());
  // 3. write file meta info
  ret = index_handle_->batch_override_segment_meta(*meta_list);
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "batch override segment meta fail. blockid: %u, ret: %d", logic_block_id_, ret);
    return ret;
  }
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "compact blockid: %u set dirty flag fail. ret: %d\n", get_logic_block_id(), ret);
    return ret;
  }
...
int LogicBlock::copy_block_info(const BlockInfo* blk_info)
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "copy block info fail. blockid: %u, ret: %d", logic_block_id_, ret);
    return ret;
  }
  return SUCCESS;
}
...
int LogicBlock::get_block_info(BlockInfo* blk_info)
{
  ...
  else
  {
    ...
    {
      ret = EXIT_POINTER_NULL;
      //LOG(ERROR, "get block info fail. blockid: %u, ret: %d", logic_block_id_, ret);
    }
    else
    {
...
int LogicBlock::extend_block(const int32_t size, const int32_t offset)
{
  ...
  while (retry_times)
  {
    if (offset + size > avail_data_size_) // need extend block
    {
      ...
      {
        //LOG(ERROR, "blockid: %u physical block list is empty!", logic_block_id_);
  ...
  if (0 == retry_times)
  {
    //LOG(ERROR, "blockid: %u extend block too much!", logic_block_id_);
    return EXIT_PHYSICALBLOCK_NUM_ERROR;
  }
  return SUCCESS;
}
...
int FileIterator::next()
{
  ...
  {
    //LOG(ERROR, "get next file, blockid: %u, id: %"PRI64_PREFIX"u, file size: %d, file offset: %d, block size: %d, read offset: %d, relative offset: %d",
  ...
  //LOG(DEBUG, "read one file end, blockid: %u, read offset: %u, leftlen: %d\n", logic_block_->logic_block_id_,
...
int FileIterator::fill_buffer()
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "batch read data fail, blockid: %u, offset: %d, read len: %d, ret: %d",
