...
int AccessControl::read_file_ip_list(const char* filename, std::set<uint32_t>& hosts)
{
  ...
  while ((read = getline(&line, &length, fp)) != -1)
  {
    ...
    if (strlen(line) < 4)
      continue;
    uint32_t server_id = CNetUtil::getAddr(line);
    if (server_id != 0)
    {
      //LOG(DEBUG, "get access_control_file line: %s", line);
...
int32_t AccessControl::load(const char* aclipmask, const char* aclfile)
{
  ...
  if (aclipmask)
  {
    ...
    for (uint32_t i = 0; i < vi.size(); ++i)
    {
      ...
      if (acip.size() != 2)
        continue;
      uint32_t ip = CNetUtil::getAddr(const_cast<char*> (acip[0].c_str()));
      uint32_t mask = CNetUtil::getAddr(const_cast<char*> (acip[1].c_str()));
      if (ip && mask)
        acl_.push_back(IpMask((ip & mask), mask));
      //LOG(DEBUG, "load access control ipmask: %u, %u", ip, mask);
    }
  }
  if (aclfile)
  {
...
{
  const char* aclipmask = NEP_CONFIG.getString(CONF_SN_DATASERVER, "access_control_ipmask");
  const char* aclfile = NEP_CONFIG.getString(CONF_SN_DATASERVER, "access_control_file");
...
int32_t AccessControl::reload()
{
  ...
  if (oneconf.load(service->config_file_.c_str()) != SUCCESS)
  {
    return ERROR;
  }
  const char* aclipmask = NEP_CONFIG.getString(CONF_SN_DATASERVER, "access_control_ipmask");
