#ifndef N_DFS_DS_INDEXHANDLE_H_
#define N_DFS_DS_INDEXHANDLE_H_
...
class IndexHandle
{
  public:
    ...
    int write_segment_meta(const uint64_t key, const RawMeta& meta);
    // read meta info
    int read_segment_meta(const uint64_t key, RawMeta& meta);
    // write metainfo; if already existed update it, or insert into the block
    int override_segment_meta(const uint64_t key, const RawMeta& meta);
    int batch_override_segment_meta(const RawMetaVec& meta_list);
    // update meta info
    int update_segment_meta(const uint64_t key, const RawMeta& meta);
    ...
    int traverse_segment_meta(RawMetaVec& raw_metas);
    int traverse_sorted_segment_meta(RawMetaVec& meta);
    ...
    BlockInfo* block_info()
    {
      ...
      if (NULL != data)
      {
        info = reinterpret_cast<BlockInfo*> (data);
      }
      return info;
    }
  ...
  private:
    ...
    int hash_insert(const int32_t slot, const int32_t previous_offset, const RawMeta& meta);
...
struct RawMetaSort
{
    bool operator()(const RawMeta& left, const RawMeta& right) const
    {
