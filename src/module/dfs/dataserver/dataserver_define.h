#ifndef N_DFS_DS_DATASERVERDEFINE_H_
#define N_DFS_DS_DATASERVERDEFINE_H_
...
static const char DEV_TAG[MAX_DEV_TAG_LEN] = "NEPTUNE";
...
struct MetaInfo
{
 public:
  ...
  explicit MetaInfo(const RawMeta& raw_meta)
  {
    raw_meta_ = raw_meta;
    next_meta_offset_ = 0;
  }
  ...
  RawMeta& get_raw_meta()
  {
    return raw_meta_;
  }

  void set_raw_meta(const RawMeta& raw_meta)
  {
    raw_meta_ = raw_meta;
  }
...
class GCObject
{
 public:
  ...
  {
    return now >= (dead_time_ + SYSPARAM_DATASERVER.object_clear_max_time_);
  }
  inline bool is_dead(const time_t now = time(NULL)) const
  {
    return now >= (dead_time_ + SYSPARAM_DATASERVER.object_dead_max_time_);
  }
 private:
  time_t dead_time_;
};
...
//typedef __gnu_cxx::hash_map<uint32_t, ReplBlock*> ReplBlockMap; // blockid => replblock
...
#endif //N_DFS_DS_DATASERVERDEFINE_H_
