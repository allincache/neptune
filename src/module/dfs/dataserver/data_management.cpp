...
void DataManagement::set_file_number(const uint64_t file_number)
{
  ...
  //LOG(INFO, "set file number. file number: %" PRI64_PREFIX "u\n", file_number_);
}

int DataManagement::init_block_files(const FileSystemParameter& fs_param)
{
  int64_t time_start = CTimeUtil::getTime();
  //LOG(INFO, "block file load blocks begin. start time: %" PRI64_PREFIX "d\n", time_start);
  // just start up
  int ret = BlockFileManager::get_instance()->bootstrap(fs_param);
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "blockfile manager boot fail! ret: %d\n", ret);
    return ret;
  }
  int64_t time_end = CTimeUtil::getTime();
  //LOG(INFO, "block file load blocks end. end time: %" PRI64_PREFIX "d. cost time: %" PRI64_PREFIX "d.",
...
int DataManagement::get_all_block_info(std::set<BlockInfo>& blocks)
{
...
int DataManagement::create_file(const uint32_t block_id, uint64_t& file_id, uint64_t& file_number)
{
  ...
  if (NULL == logic_block)
  {
    //LOG(ERROR, "blockid: %u is not exist.", block_id);
  ...
  if (0 == file_id)
  {
    ...
    if (SUCCESS != ret)
    {
      //LOG(ERROR, "try getfileid failed. blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d.", block_id,
      //    file_id, ret);
      return ret;
    }
  }
  ...
  //LOG(DEBUG, "open write file. blockid: %u, fileid: %" PRI64_PREFIX "u, filenumber: %" PRI64_PREFIX "u",
    ...
    const char* data_buffer, UpdateBlockType& repair)
{
  ...
  if (0 == write_info.offset_)
  {
    ...
    if (NULL == logic_block)
    {
      //LOG(ERROR, "blockid: %u is not exist.", write_info.block_id_);
      return EXIT_NO_LOGICBLOCK_ERROR;
    }
    ...
    if (SUCCESS != ret)
    {
      //LOG(DEBUG, "check_block_version error. blockid: %u, ret: %d", write_info.block_id_, ret);
      return ret;
    }
  }
  ...
  else                      // not found
  {
    ...
    {
      //LOG(ERROR, "blockid: %u, datafile nums: %zd is large than default.", write_info.block_id_,
      //    data_file_map_.size());
      data_file_mutex_.unlock();
  ...
  if (NULL == datafile)
  {
    //LOG(ERROR, "datafile is null. blockid: %u, fileid: %" PRI64_PREFIX "u, filenumber: %" PRI64_PREFIX "u",
...
int DataManagement::close_write_file(const CloseFileInfo& colse_file_info, int32_t& write_file_size)
{
  ...
  {
    //LOG(WARN, "write file cost time: blockid: %u, fileid: %" PRI64_PREFIX "u, cost time: %" PRI64_PREFIX "d",
    ...
    int32_t& real_read_len, char* tmp_data_buffer)
{
  ...
  if (NULL == logic_block)
  {
    //LOG(ERROR, "block not exist, blockid: %u", block_id);
    return EXIT_NO_LOGICBLOCK_ERROR;
  }

  int64_t start = CTimeUtil::getTime();
  ...
  //LOG(DEBUG, "blockid: %u read data, fileid: %" PRI64_PREFIX "u, read size: %d, offset: %d", block_id,
  //    file_id, real_read_len, read_offset);

  int64_t end = CTimeUtil::getTime();
  if (end - start > SYSPARAM_DATASERVER.max_io_warn_time_)
  {
    //LOG(WARN, "read file cost time: blockid: %u, fileid: %" PRI64_PREFIX "u, cost time: %" PRI64_PREFIX "d",
    ...
    char* tmp_data_buffer)
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "blockid: %u read data batch error, offset: %d, rlen: %d, ret: %d", block_id, read_offset,
    //    real_read_len, ret);
    return ret;
  }
...
int DataManagement::rename_file(const uint32_t block_id, const uint64_t file_id, const uint64_t new_file_id)
{
  ...
  {
    //LOG(WARN, "rename file fail. blockid:%u, fileid: %" PRI64_PREFIX "u, newfileid: %" PRI64_PREFIX "u",
...
int DataManagement::unlink_file(const uint32_t block_id, const uint64_t file_id, const int32_t action, int64_t& file_size)
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "del file fail, blockid: %u, fileid: %" PRI64_PREFIX "u, ret: %d", block_id, file_id, ret);
    return ret;
  }
...
int DataManagement::batch_new_block(const VUINT32* new_blocks)
{
  ...
  if (NULL != new_blocks)
  {
    ...
    {
      //LOG(INFO, "new block: %u\n", new_blocks->at(i));
      uint32_t physic_block_id = 0;
      ret = BlockFileManager::get_instance()->new_block(new_blocks->at(i), physic_block_id);
      if (SUCCESS != ret)
      {
        //LOG(ERROR, "new block fail, blockid: %u, ret: %d", new_blocks->at(i), ret);
        return ret;
      }
      else
      {
        //LOG(INFO, "new block successful, blockid: %u, phyical blockid: %u", new_blocks->at(i),
        //    physic_block_id);
      }
    }
  }
...
int DataManagement::batch_remove_block(const VUINT32* remove_blocks)
{
  ...
  if (NULL != remove_blocks)
  {
    ...
    {
      //LOG(INFO, "remove block: %u\n", remove_blocks->at(i));
      ret = BlockFileManager::get_instance()->del_block(remove_blocks->at(i));
      if (SUCCESS != ret)
      {
        //LOG(ERROR, "remove block error, blockid: %u, ret: %d", remove_blocks->at(i), ret);
        return ret;
      }
      else
      {
        //LOG(INFO, "remove block successful, blockid: %u", remove_blocks->at(i));
      }
    }
  }
    ...
    uint32_t> >& logic_2_physic_blocks, std::map<uint32_t, BlockInfo*>& block_2_info)
{
  ...
  if (query_type & LB_PAIRS) // logick block ==> physic block list
  {
    ...
    for (lit = logic_blocks.begin(); lit != logic_blocks.end(); ++lit)
    {
      if (*lit)
      {
        //LOG(DEBUG, "query block status, query type: %d, blockid: %u\n", query_type,
...
{
  //LOG(INFO, "getfilelist. blockid: %u\n", block_id);
  LogicBlock* logic_block = BlockFileManager::get_instance()->get_logic_block(block_id);
  if (NULL == logic_block)
  {
    //LOG(ERROR, "blockid: %u is not exist.", block_id);
...
int DataManagement::get_block_meta_info(const uint32_t block_id, RawMetaVec& meta_list)
{
  ...
  //LOG(INFO, "get meta list. blockid: %u, filenum: %zd\n", block_id, meta_list.size());
  return SUCCESS;
}
...
int DataManagement::reset_block_version(const uint32_t block_id)
{
  ...
  //LOG(INFO, "reset block version: %u\n", block_id);
  return SUCCESS;
}
...
int DataManagement::new_single_block(const uint32_t block_id, const BlockType type)
{
  ...
  if (NULL != logic_block)
  {
    //LOG(INFO, "block already exist, blockid: %u. first del it", block_id);
    ret = BlockFileManager::get_instance()->del_block(block_id);
    if (SUCCESS != ret)
    {
      //LOG(ERROR, "block already exist, blockid: %u. block delete fail. ret: %d", block_id, ret);
      return ret;
    }
  }
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "block create error, blockid: %u, ret: %d", block_id, ret);
    return ret;
  }
  return SUCCESS;
}
...
{
  //LOG(INFO, "remove single block, blockid: %u", block_id);
...
int DataManagement::get_block_curr_size(const uint32_t block_id, int32_t& size)
{
  ...
  //LOG(DEBUG, "blockid: %u data file size: %d\n", block_id, size);
  return SUCCESS;
}
    ...
    const char* data_buffer)
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "write data batch error. blockid: %u, ret: %d", block_id, ret);
    return ret;
  }
...
int DataManagement::batch_write_meta(const uint32_t block_id, const BlockInfo* blk, const RawMetaVec* meta_list)
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "blockid: %u batch write meta error.", block_id);
    return ret;
  }
...
int DataManagement::gc_data_file()
{
  ...
  if (last_gc_data_file_time_ < diff_time)
  {
    ...
    data_file_mutex_.unlock();
    //LOG(INFO, "datafilemap size. old: %d, new: %d", old_data_file_size, new_data_file_size);
  }
