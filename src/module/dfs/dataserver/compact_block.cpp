...
int CompactBlock::run_compact_block()
{
  while (!stop_)
  {
    ...
    //LOG(INFO, "start compact block. blockid: %d", cpt_blk->block_id_);
    ...
    if (SUCCESS != ret)
    {
      ...
      if (SUCCESS != del_ret)
      {
        //LOG(ERROR, "compact blockid: %u, del old block error ret: %d\n", cpt_blk->block_id_, del_ret);
      }
    }

    int64_t end_time = Func::curr_time();
    //LOG(INFO, "finish compact. blockid: %u, cost time: %" PRI64_PREFIX "u, ret: %d", cpt_blk->block_id_,
...
int CompactBlock::add_cpt_task(CompactBlkInfo* cpt_blk)
{
  ...
  //LOG(INFO, "add compact block. cpt_exist: %d, blockid: %u, owner: %d, preserve_time: %d\n",
...
int CompactBlock::real_compact(const uint32_t block_id)
{
  ...
  if (SUCCESS != ret)
    return ret;
  //LOG(DEBUG, "compact new block blockid: %u, physical blockid: %d\n", block_id, physical_block_id);
  ...
  {
    //LOG(ERROR, "get compact dest block fail. blockid: %u\n", block_id);
    return EXIT_NO_LOGICBLOCK_ERROR;
  }
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "inner real compact blockid: %u fail. ret: %d\n", block_id, ret);
    return ret;
  }
  //LOG(DEBUG, "compact blockid : %u, switch compact blk\n", block_id);

  // switch the compact block with serve block
  ret = BlockFileManager::get_instance()->switch_compact_blk(block_id);
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "compact blockid: %u, switch compact blk fail. ret: %d\n", block_id, ret);
    return ret;
  }

  //LOG(DEBUG, "compact del old blockid: %u\n", block_id);
  // del serve block
  ret = BlockFileManager::get_instance()->del_block(block_id, C_COMPACT_BLOCK);
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "compact blockid: %u after switch, del old block fail. ret: %d\n", block_id, ret);
  }
...
int CompactBlock::req_block_compact_complete(const uint32_t block_id, const int32_t success, const int64_t seqno)
{
  ...
  if (NULL == LogicBlock)
  {
    //LOG(ERROR, "get block failed. blockid: %u\n", block_id);
    return EXIT_NO_LOGICBLOCK_ERROR;
  }
...
int CompactBlock::clear_compact_block_map()
{
  ...
  for (uint32_t i = 0; i < compact_blocks.size(); i++)
  {
    ...
    if (SUCCESS != ret)
    {
      //LOG(ERROR, "in check thread: del compact blockid: %u fail. ret: %d", compact_blocks[i], ret);
    }
  }
  return SUCCESS;
}
...
int CompactBlock::real_compact(LogicBlock* src, LogicBlock* dest)
{
  ...
  //LOG(DEBUG, "compact block set last update. blockid: %u\n", dest->get_logic_block_id());
  ...
  while (fit->has_next())
  {
    ...
    {
      //LOG(DEBUG, "write one, blockid: %u, write offset: %d\n", dest->get_logic_block_id(),
      //    write_offset);
      ret = dest->write_raw_data(dest_buf, data_len, write_offset);
      if (SUCCESS != ret)
      {
        //LOG(ERROR, "write raw data fail, blockid: %u, offset %d, readinglen: %d, ret :%d",
  ...
  gDeleteA(dest_buf);
  gDelete(fit);
  //LOG(DEBUG, "compact write complete. blockid: %u\n", dest->get_logic_block_id());

  ret = dest->batch_write_meta(&dest_blk, &dest_metas);
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "compact write segment meta failed. blockid: %u, meta size %zd\n", dest->get_logic_block_id(),
    //    dest_metas.size());
    return ret;
  }

  //LOG(DEBUG, "compact set dirty flag. blockid: %u\n", dest->get_logic_block_id());
    ...
    const FileInfo& dest_info, int32_t woffset)
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "write big file error, blockid: %u, ret: %d", dest->get_logic_block_id(), ret);
  }
  return ret;
}
