...
int BlockChecker::consume_repair_task()
{
  ...
  //LOG(INFO, "start repair blockid: %u, fileid: %" PRI64_PREFIX "u", ccf->block_id_, ccf->file_id_);
...
int BlockChecker::do_repair_crc(const CrcCheckFile& check_file)
{
  ...
  if (!init_status)
  {
    //LOG(ERROR, "file repair init fail.\n");
    return ERROR;
  }
  ...
  if (SUCCESS != ret)
  {
    check_mutex_.unlock();
    unlink(tmp_file);
    //LOG(ERROR, "fetch file fail. ret: %d, unlink tmp file: %s\n", ret, tmp_file);
    return ret;
  }
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "repair file fail. ret: %d\n", ret);
    set_error_bitmap(logic_block);
    check_mutex_.unlock();
    return ds_requester_->req_update_block_info(check_file.block_id_, UPDATE_BLOCK_REPAIR);
  }
...
int BlockChecker::expire_error_block()
{
  ...
  if (SUCCESS != ret)
  {
    //LOG(ERROR, "list logic block fail ret: %d", ret);
  }

  //LOG(INFO, "BlockChecker get logic block size: %zd", logic_blocks.size());
  ...
  int64_t end_time = Func::curr_time();
  //LOG(INFO, "end expire error block expire blocks: %d, cost time: %" PRI64_PREFIX "d", expire_block_nums,
  //    (end_time - start_time));
  return SUCCESS;
}
...
int BlockChecker::check_block(const LogicBlock* logic_block)
{
  ...
  {
    //LOG(ERROR, "block may be error, will be removed. blockid: %u, crc error nums: %d, eio error nums: %d\n",
