...
int ClientRequestServer::keepalive(const DataServerStatInfo& ds_info, const time_t now)
{
  int32_t ret = ERROR;
  ...
  else
  {
    ...
    if (ret == SUCCESS)
    {
      ...
      ret = (NULL == server) ? EIXT_SERVER_OBJECT_NOT_FOUND : SUCCESS;
      if (SUCCESS == ret)
      {
        if (isnew) //new dataserver
        {
          //LOG(INFO, "dataserver: %s join: use capacity: %" PRI64_PREFIX "u, total capacity: %" PRI64_PREFIX "u",
        ...
        if (server->is_report_block(rb_expire, now, isnew))
        {
          ...
          //LOG(DEBUG, "%s add report block server, now: %ld, isnew: %d, rb_expire: %d",
          //  CNetUtil::addrToString(server->id()).c_str(), now, isnew, rb_expire);
    ...
    std::set<BlockInfo>& blocks)
{
  ...
  ret = NULL == pserver ? EIXT_SERVER_OBJECT_NOT_FOUND : SUCCESS;
  if (SUCCESS == ret)
  {
    //update all relations of blocks belongs to it
    ret = manager_.update_relation(pserver, blocks, now);
    ret = SUCCESS != ret ?  EXIT_UPDATE_RELATION_ERROR : SUCCESS;
    if (SUCCESS == ret)
    {
    ...
    VUINT64& servers, const int32_t mode, const time_t now)
{
  ...
  {
    ret = GFactory::get_runtime_info().is_master() ? SUCCESS : EXIT_ACCESS_PERMISSION_ERROR;
    if (SUCCESS == ret)//master
    {
          ...
          && (!(mode & T_NOLEASE)))
      {
        ...
        {
          //LOG(INFO, "it's error when we'll get block information in open this block: %u with write mode because block: %u is busy.",
          //    block_id,  mode);
          ret = EXIT_BLOCK_BUSY;
        }
      }
  ...
  mode & T_READ ? ret == SUCCESS ? stat[0] = 1 : stat[1] = 1
    : ret == SUCCESS ? stat[2] = 1 : stat[3] = 1;
  GFactory::get_stat_mgr().update_entry(GFactory::dfs_ns_stat_, stat);
  return ret;
}

int ClientRequestServer::open_read_mode_(VUINT64& servers, const uint32_t block) const
{
  servers.clear();
  return 0 == block ? EXIT_BLOCK_NOT_FOUND :  manager_.get_block_manager().get_servers(servers, block);
}

int ClientRequestServer::batch_open(const VUINT32& blocks, const int32_t mode, const int32_t block_count, std::map<uint32_t, BlockInfoSeg>& out)
{
...
/**
 * Write commit operation, metaserver confirm this write op and update meta info.
...
int ClientRequestServer::close(CloseParameter& parameter)
{
  ...
  else //write file
  {
    ...
    {
      //LOG(INFO, "close block: %u successful, but cleint write operation error,lease: %u",
    ...
    else
    {
      BlockCollect* block = manager_.get_block_manager().get(block_id);
      ret = (NULL == block) ? EXIT_BLOCK_NOT_FOUND : SUCCESS;
      if (SUCCESS == ret)//check version
      {
        ret = block->version() >= parameter.block_info_.version_ ? EXIT_COMMIT_ERROR : SUCCESS;
        if (SUCCESS != ret)
        {
...
{
  int32_t ret = (mode & T_WRITE) ? SUCCESS : EXIT_ACCESS_MODE_ERROR;
  if (SUCCESS != ret)
  {
    //LOG(WARN, "access mode: %d error", mode);
  }
  else
  {
    //metaserver assign a new write block
    ...
    if (mode & T_CREATE)
    {
      if (0 == block_id)
      {
        ...
        ret = (NULL == block) ? EXIT_NO_BLOCK : SUCCESS;
        if (SUCCESS == ret)
          block_id = block->id();
        else
          {}
          //LOG(INFO, "%s", "it's failed when choose a write block");
      }
    }
    ...
    if ((mode & T_NEWBLK)
        && (NULL == block))
    {
      ret = 0 == block_id ? EXIT_BLOCK_ID_INVALID_ERROR: SUCCESS;
      if (SUCCESS == ret)
      {
        time_t now = Func::get_monotonic_time();
        NsRuntimeGlobalInformation& ngi = GFactory::get_runtime_info();
        ret = ngi.in_discard_newblk_safe_mode_time(now) || is_discard() ? EXIT_DISCARD_NEWBLK_ERROR: SUCCESS;
        if (SUCCESS == ret)
        {
          ...
          if (SUCCESS != ret)
            {}
            //LOG(INFO, "create new block by block id: %u failed", block_id);
        }
      }
    }
    ...
    {
      ret = block_id == 0 ? EXIT_NO_BLOCK : SUCCESS;
      if (SUCCESS == ret)
      {
        block =  manager_.get_block_manager().get(block_id);
        ...
        if (SUCCESS == ret)
        {
          ...
          ret = !servers.empty() ? SUCCESS : EXIT_NO_DATASERVER;
        }
      }
    }
...
int ClientRequestServer::batch_open_read_mode_(std::map<uint32_t, BlockInfoSeg>& out, const VUINT32& blocks) const
{
  std::pair<std::map<uint32_t, BlockInfoSeg>::iterator, bool> res;
  VUINT32::const_iterator iter = blocks.begin();
  for (; iter != blocks.end(); ++iter)
  {
    res = out.insert(std::make_pair((*iter), BlockInfoSeg()));
    open_read_mode_(res.first->second.ds_, (*iter));
  }
  return SUCCESS;
}

int ClientRequestServer::batch_open_write_mode_(std::map<uint32_t, BlockInfoSeg>& out,const int32_t mode, const int32_t block_count)
{
  int32_t ret =  (mode & T_WRITE) ? SUCCESS : EXIT_ACCESS_MODE_ERROR;
  if (SUCCESS == ret)
  {
...
int ClientRequestServer::handle_control_load_block(const time_t now, const ClientCmdInformation& info, BasePacket* message, const int64_t buf_length, char* buf)
{
  //LOG(INFO, "handle control load block: %u, server: %s",
  //    info.value3_, CNetUtil::addrToString(info.value1_).c_str());
  int32_t ret = ((NULL != buf) && (buf_length > 0)) ? SUCCESS : EXIT_PARAMETER_ERROR;
  if (SUCCESS == ret)
  {
    ...
    {
      ret = manager_.get_block_manager().get_servers_size(block) <= 0 ? SUCCESS : EXIT_BLOCK_REPLICATE_EXIST;
    }
    ret  = NULL == block ? EXIT_NO_BLOCK : SUCCESS;
    if (SUCCESS != ret)
    {
    ...
    else
    {
      ...
      else
      {
        ...
        if (SUCCESS == ret)
        {
          ret = manager_.build_relation(block, server, now, true) ? SUCCESS : EXIT_BUILD_RELATION_ERROR;
        }
      }
      GCObject* pobject = NULL;
...
int ClientRequestServer::handle_control_delete_block(const time_t now, const ClientCmdInformation& info,const int64_t buf_length, char* buf)
{
  //LOG(INFO, "handle control remove block: %u, flag: %u, server: %s",
  ...
  if (SUCCESS == ret)
  {
    ...
    else
    {
      ...
      {
        ServerCollect* server = manager_.get_server_manager().get(info.value1_);
        ret = NULL != server ? SUCCESS : EIXT_SERVER_OBJECT_NOT_FOUND;
        if (SUCCESS == ret)
...
int ClientRequestServer::handle_control_compact_block(const time_t now, const ClientCmdInformation& info, const int64_t buf_length, char* buf)
{
  //LOG(INFO, "handle control compact block: %u", info.value3_);
  int32_t ret = ((NULL == buf) || (buf_length <= 0)) ? EXIT_PARAMETER_ERROR : SUCCESS;
  if (SUCCESS == ret)
  {
    std::vector<ServerCollect*> runer;
    ret = manager_.get_block_manager().get_servers(runer, info.value3_);
...
int ClientRequestServer::handle_control_immediately_replicate_block(const time_t now, const ClientCmdInformation& info, const int64_t buf_length, char* buf)
{
  //LOG(INFO, "handle control %s block: %u, source: %s, target: %s",
  ...
  if (SUCCESS == ret)
  {
    ...
    if (SUCCESS != ret)
    {
      ret = 0 == info.value3_ ? EXIT_BLOCK_ID_INVALID_ERROR : SUCCESS;
      if (SUCCESS == ret)
      {
        new_create_block_collect = true;
        block = manager_.get_block_manager().insert(info.value3_, now, true);
    ...
    if (SUCCESS == ret)
    {
      ...
      ret = NULL != source ? SUCCESS : EXIT_NO_DATASERVER;
      if (SUCCESS != ret)
      {
      ...
      else
      {
        ...
        ret = NULL != target ? SUCCESS : EXIT_NO_DATASERVER;
        if (SUCCESS != ret)
        {
      ...
      if (SUCCESS == ret)//NULL != source && NULL != target
      {
        std::vector<ServerCollect*> runer;
...
int ClientRequestServer::handle_control_rotate_log(void)
{
  ...
  LOGGER.rotateLog(service->get_log_path());
  return SUCCESS;
}

int ClientRequestServer::handle_control_set_runtime_param(const ClientCmdInformation& info, const int64_t buf_length, char* buf)
{
...
int ClientRequestServer::handle_control_set_balance_percent(const ClientCmdInformation& info, const int64_t buf_length, char* buf)
{
  int32_t ret = (info.value3_ > 1 || info.value3_ < 0 || info.value4_ < 0) ? EXIT_PARAMETER_ERROR : SUCCESS;
  if (SUCCESS != ret)
  {
...
int ClientRequestServer::handle_control_clear_system_table(const ClientCmdInformation& info, const int64_t buf_length, char* buf)
{
  int32_t ret = (info.value3_ <= 0) ? EXIT_PARAMETER_ERROR : SUCCESS;
  if (SUCCESS != ret)
  {
...
int ClientRequestServer::handle_control_cmd(const ClientCmdInformation& info, BasePacket* msg, const int64_t buf_length, char* buf)
{
  time_t now = Func::get_monotonic_time();
  int32_t ret = ERROR;
  switch (info.cmd_)
...
int ClientRequestServer::handle(BasePacket* msg)
{
  ...
  if (SUCCESS == ret)
  {
    ...
    switch (pcode)
    {
      ...
      case REMOVE_BLOCK_RESPONSE_MESSAGE:
        ...
        break;
      default:
        //LOG(WARN, "unkonw message PCode = %d", pcode);
        break;
    }
  }
  return ret;
...
void ClientRequestServer::dump_plan(DataBuffer& output)
{
