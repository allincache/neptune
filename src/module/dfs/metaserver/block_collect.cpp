...
bool BlockCollect::add(bool& writable, bool& master, ServerCollect*& invalid_server, const ServerCollect* server)
{
  ...
  if (ret)
  {
    ...
    if (NULL == result)//not found
    {
      ...
      //LOG(DEBUG, "random_index : %d, servers_size: %d", random_index, get_servers_size());
...
{
  ////LOG(DEBUG, "is_full: %d, size: %d", is_full(), get_servers_size());
...
bool BlockCollect::check_version(LayoutManager& manager, ArrayHelper<ServerCollect*>& removes,
    bool& expire_self, ArrayHelper<ServerCollect*>& other_expires, ServerCollect*& invalid_server,
    const ServerCollect* server,const int8_t role, const bool isnew, const BlockInfo& info, const time_t now)
{
  expire_self = false;
  invalid_server = NULL;
  ...
  if (ret)
  {
    ...
    if (ret)
    {
      ...
      if (ret)
      {
        ...
        {
          if (info_.version_ > info.version_)// metaserver version > dataserver version
          {
            if (size > 0)//has dataserver hold block, release
            {
              //LOG(INFO, "block: %u in dataserver: %s version error %d:%d",
              //    info.block_id_, CNetUtil::addrToString(server->id()).c_str(),
              //    info_.version_, info.version_);
              expire_self = (role == NS_ROLE_MASTER);
              ret = false;
            }
            ...
            {
              //LOG(WARN, "block: %u in dataserver: %s version error %d:%d, but not found dataserver",
              //    info.block_id_, CNetUtil::addrToString(server->id()).c_str(),
              //    info_.version_, info.version_);
              info_ = info;
              //memcpy(&info_,&info, sizeof(info_));
          ...
          else if ( info_.version_ < info.version_) // metaserver version < dataserver version , we'll accept new version and release all dataserver
          {
            ...
            {
              //LOG(INFO, "block: %u in dataserver: %s version error %d:%d,replace ns version, current dataserver size: %u",
              //    info.block_id_, CNetUtil::addrToString(server->id()).c_str(),
              ...
              if (role == NS_ROLE_MASTER)
              {
                ...
                for (int8_t i = 0; i < SYSPARAM_NAMESERVER.max_replication_; ++i)
                {
                  ...
                  if (NULL != pserver)
                  {
                    //LOG(INFO, "release relation dataserver: %s, block: %u",
                    //    CNetUtil::addrToString(pserver->id()).c_str(), info_.block_id_);
...
PlanPriority BlockCollect::check_replicate(const time_t now) const
{
  ...
  if (BLOCK_CREATE_FLAG_YES != create_flag_)
  {
    ...
    ////LOG(DEBUG, "replicate block %u, size: %d, time: %ld, now: %ld",
    //  id(), size, last_update_time_, now);
    if (size <= 0)
    {
      //LOG(WARN, "block: %u has been lost, do not replicate", info_.block_id_);
    }
    else if (size == 1 && SYSPARAM_NAMESERVER.max_replication_ > 1)
    {
      if (last_update_time_ + SYSPARAM_NAMESERVER.replicate_wait_time_ <= now)
      {
        //LOG(DEBUG, "emergency replicate block: %u", info_.block_id_);
    ...
    else if (size < SYSPARAM_NAMESERVER.max_replication_)
    {
      if (last_update_time_ + SYSPARAM_NAMESERVER.replicate_wait_time_ <= now)
      {
        //LOG(DEBUG, "replicate block: %u", info_.block_id_);
  ...
  if (BLOCK_CREATE_FLAG_YES != create_flag_)
  {
    ...
    if ((size == SYSPARAM_NAMESERVER.max_replication_) && is_full())
    {
          ...
          && (info_.del_size_ > 0))
      {
        ...
        {
          //LOG(DEBUG, "block: %u need compact", info_.block_id_);
          bret = true;
        }
      }
    }
