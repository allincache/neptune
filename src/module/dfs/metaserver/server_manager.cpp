...
int ServerManager::add(const DataServerStatInfo& info, const time_t now, bool& isnew)
{
  ...
  GFactory::get_global_info().dump(LOG_LEVEL_DEBUG);
  return ret;
}
...
int ServerManager::remove(const uint64_t server, const time_t now)
{
  ...
  if (NULL != object)
  {
    //LOG(INFO, "dataserver: %s exit, now: %"PRI64_PREFIX"d, last_update_time: %"PRI64_PREFIX"d",
    //    CNetUtil::addrToString(server).c_str(), now, object->get_last_update_time());
...
int ServerManager::add_report_block_server(ServerCollect* server, const time_t now, const bool rb_expire)
{
  ...
  {
    ServerCollect* result = NULL;
    Mutex::Lock lock(wait_report_block_server_mutex_);
    if (rb_expire)
...
int ServerManager::get_and_move_report_block_server(ArrayHelper<ServerCollect*>& helper, const int64_t max_slot_num)
{
  Mutex::Lock lock(wait_report_block_server_mutex_);
    ...
    SERVER_TABLE& targets, const ServerCollect* server, const double percent) const
{
  if (NULL != server)
  {
    ...
    //LOG(DEBUG, "move_split_server: %s, current_percent: %e, balance_percent: %e, percent: %e",
    //   CNetUtil::addrToString(server->id()).c_str(), current_percent, SYSPARAM_NAMESERVER.balance_percent_, percent);
        ...
        || (current_percent >= 1.0))
    {
      ...
      //LOG(DEBUG, "move_split_server: %s, %ld",
      // CNetUtil::addrToString(server->id()).c_str(), static_cast<int64_t>(current_percent * PERCENTAGE_MAGIC));
    }
    else
    {
...
int ServerManager::scan(SSMScanParameter& param, int32_t& should, int32_t& start, int32_t& next, bool& all_over) const
{
  int32_t actual= 0;
...
{
  int32_t ret = ((NULL != server) && (NULL != block)) ? SUCCESS : EXIT_PARAMETER_ERROR;
  if (SUCCESS == ret)
  {
...
int ServerManager::choose_writable_server_lock_(ServerCollect*& result)
{
  result = NULL;
  RWLock::Lock lock(rwmutex_, READ_LOCKER);
  int32_t ret = servers_.empty() ? EXIT_NO_DATASERVER : SUCCESS;
  if (SUCCESS == ret)
  {
  ...
  return NULL != result ? SUCCESS : EXIT_NO_DATASERVER;
}
...
int ServerManager::choose_writable_server_random_lock_(ServerCollect*& result)
{
  result = NULL;
  RWLock::Lock lock(rwmutex_, READ_LOCKER);
  int32_t ret = !servers_.empty() ? SUCCESS : EXIT_NO_DATASERVER;
  if (SUCCESS == ret)
  {
...
int ServerManager::choose_create_block_target_server(ArrayHelper<ServerCollect*>& result,
...
{
  result = NULL;
  int32_t ret = !source.empty() ? SUCCESS : EXIT_NO_DATASERVER;
  if (SUCCESS == ret)
  {
  ...
  return NULL == result ? EXIT_NO_DATASERVER : SUCCESS;
}
    ...
    const ArrayHelper<ServerCollect*>& except) const
{
  ...
  return ((ret == SUCCESS) && (NULL != result)) ? SUCCESS : EXIT_NO_DATASERVER;
}
    ...
    ArrayHelper<ServerCollect*>& except) const
{
  ...
  while (index--> 0 && NULL == result)
  {
    ...
    assert(NULL != pserver);
    uint32_t lan =  Func::get_lan(pserver->id(), SYSPARAM_NAMESERVER.group_mask_);
    //LOG(DEBUG, "==============addr: %s, lans : %u", CNetUtil::addrToString(pserver->id()).c_str(), lan);
...
int ServerManager::choose_excess_backup_server(ServerCollect*& result, const ArrayHelper<ServerCollect*>& sources) const
{
  result = NULL;
  SORT_MAP sorts;
...
int ServerManager::expand_ratio(int32_t& index, const float expand_ratio)
{
  ...
  rwmutex_.unlock();
  return NULL != server ? server->expand_ratio(expand_ratio) : SUCCESS;
}
...
void ServerManager::get_lans_(std::set<uint32_t>& lans, const ArrayHelper<ServerCollect*>& source) const
{
  ServerCollect* server = NULL;
  if (!source.empty())
  {
    for (int32_t i = 0; i < source.get_array_index(); ++i)
    {
      ...
      assert(NULL != server);
      uint32_t lan =  Func::get_lan(server->id(), SYSPARAM_NAMESERVER.group_mask_);
      //LOG(DEBUG, "addr: %s, lans : %u", CNetUtil::addrToString(server->id()).c_str(), lan);
      lans.insert(lan);
    }
  }
}
...
bool ServerManager::relieve_relation_(ServerCollect* server, const time_t now)
{
  ...
  if (ret)
  {
    ...
    ArrayHelper<BlockCollect*> helper(MAX_SLOT_NUMS, blocks);
    bool complete = false;
