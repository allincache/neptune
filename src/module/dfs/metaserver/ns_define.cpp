...
void NsGlobalStatisticsInfo::update(const DataServerStatInfo& info, const bool is_new)
{
...
{
//  //LOGGER.logMessage(level, file, line, function,
...
{
  last_owner_check_time_ = Time::now(Time::Monotonic).toMicroSeconds() + SYSPARAM_NAMESERVER.safe_mode_time_ * 1000000;
  ...
  {
    startup_time_ = now;
    switch_time_  = now + SYSPARAM_NAMESERVER.safe_mode_time_;
    discard_newblk_safe_mode_time_ =  now + SYSPARAM_NAMESERVER.discard_newblk_safe_mode_time_;
    peer_role_ = NS_ROLE_SLAVE;
    owner_role_ = Func::is_local_addr(vip_) == true ? NS_ROLE_MASTER : NS_ROLE_SLAVE;
    //LOG(INFO, "i %s the master server", owner_role_ == NS_ROLE_MASTER ? "am" : "am not");
  }
  else//switch
  {
    ...
    if (now - SYSPARAM_NAMESERVER.safe_mode_time_ > startup_time_)
    {
      switch_time_  = now + (SYSPARAM_NAMESERVER.safe_mode_time_ / 2);
    }
    ...
    //LOG(INFO, "i %s the master server, switch time: %"PRI64_PREFIX"d", owner_role_ == NS_ROLE_MASTER ? "am" : "am not", switch_time_);
  }
}
      ...
      const int8_t role, const int8_t status, const int8_t type, const time_t now)
{
  ...
  int32_t ret = owner_role_ == NS_ROLE_MASTER ? SUCCESS : EXIT_ROLE_ERROR;
  if (SUCCESS == ret)
  {
    ...
    {
      peer_ip_port_ = server;
      lease_id = lease_id_ = atomic_inc(&lease_id_factory);
      renew(now, SYSPARAM_NAMESERVER.heart_interval_);
    }
    else if (NS_KEEPALIVE_TYPE_RENEW == type)
    {
      ret = INVALID_LEASE_ID == lease_id_ ? EXIT_LEASE_EXPIRED : SUCCESS;
      if (SUCCESS == ret)
      {
        ret = lease_id_ == lease_id ? SUCCESS : EXIT_RENEW_LEASE_ERROR;
        if (SUCCESS == ret)
        {
          renew(now, SYSPARAM_NAMESERVER.heart_interval_);
        }
  ...
  //LOG(DEBUG, "peer_role: %d, peer_status: %d,status: %d, lease_id: %ld, %ld, type: %d, time: %ld,now: %ld",
...
{
  //LOG(DEBUG, "Now: %ld, lease id: %ld, lease_expired_time: %ld", now, lease_id_, lease_expired_time_);
...
, last owner check time: %s, last push owner check packet time: %s",
      NULL == format ? "" : format,
      CNetUtil::addrToString(owner_ip_port_).c_str(),
      CNetUtil::addrToString(peer_ip_port_).c_str(),
      Func::time_to_str(switch_time_).c_str(), CNetUtil::addrToString(vip_).c_str(),
      ...
      Func::time_to_str(last_owner_check_time_/1000000).c_str(),
      Func::time_to_str(last_push_owner_check_packet_time_/1000000).c_str());*/
}
...
std::string& print_servers(const ArrayHelper<ServerCollect*>&servers, std::string& result)
{
  ...
  for (int32_t i = 0; i < servers.get_array_index(); ++i)
  {
    server = *servers.at(i);
    result += "/";
    result += CNetUtil::addrToString(server->id());
  }
  return result;
}

void print_servers(const ArrayHelper<uint64_t>& servers, std::string& result)
{
  for (int32_t i = 0; i < servers.get_array_index(); ++i)
  {
    result += "/";
    result += CNetUtil::addrToString(*servers.at(i));
  }
}
...
void print_servers(const std::vector<uint64_t>& servers, std::string& result)
{
  ...
  {
    result += "/";
    result += CNetUtil::addrToString((*iter));
  }
}
