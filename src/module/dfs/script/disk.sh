...
DFS_HOME="`cd ..;pwd`"
DFS_CONF=/opt/neptune/dfs/conf/dataserver.conf
BIN_DIR=${DFS_HOME}/script
CLEAR_BIN=${BIN_DIR}/clear_file_system
FORMAT_BIN=${BIN_DIR}/format_file_system
DS_CMD="${BIN_DIR}/dataserver -f ${DFS_CONF} -d -i"
WAIT_TIME=5
...
do_ds()
{
    case "$1" in
	format)
	    cmd="$FORMAT_BIN -f $DFS_CONF -i"
	    name="format ds"
	    ;;
	clear)
	    cmd="$CLEAR_BIN -f $DFS_CONF -i"
	    name="clear ds"
	    ;;
	*)
